...
{
    v[0xF] = v[get_x(opcode)] >= v[get_y(opcode)]; // <-- CORREGIDO: >= en lugar de >
    v[get_x(opcode)] -= v[get_y(opcode)];
}
...
{
    v[0xF] = v[get_y(opcode)] >= v[get_x(opcode)]; // <-- CORREGIDO: >= en lugar de >
...
{
    st = v[get_x(opcode)]; // <-- CORREGIDO: st en lugar de dt
}
